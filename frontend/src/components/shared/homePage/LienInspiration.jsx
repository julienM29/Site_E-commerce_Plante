...
const LienInspiration = ({ image, texte, link }) => {
  ...
  return (
    <Link to={`${link}`} className='relative flex justify-center group lienInspiration'>
  <img
    src={image}
    alt=""
    className='rounded-2xl h-52 md:h-72 w-96 transform transition-transform duration-300 group-hover:scale-105 group-hover:brightness-75'
  />
